...
//#define AUTO_POWER_ON_OFF_FLAG //for auto power-onoff test 2011-03-08 hc.hyun
...
{
	struct pmic8058_kp *kp = (struct pmic8058_kp *)data;
#ifdef KERNEL_DEBUG_SEC
	printk("force to press powerkey.\n");
#endif
	input_report_key(kp->input, KEY_END, 1);
}
#endif
...
static int __devinit pmic8058_kp_probe(struct platform_device *pdev)
{
	...
	for(i = 0 ; i < ARRAY_SIZE(mmi_keycode) ; i++)
		input_set_capability(kp->input,EV_KEY, mmi_keycode[i]); 	
#endif
...
static int pmic8058_kp_suspend(struct device *dev)
{
	...
	if (device_may_wakeup(dev) && !pmic8058_kp_disabled(kp)) {
		if(on_call_flag || on_fmradio_flag) {
