...
static unsigned CFG0_A, CFG1_A, ECC_CFG_A;
#endif
...
uint32_t flash_onfi_probe(struct msm_nand_chip *chip)
{
	...
	for ( i = 0 ; i < msm_nand_data.nr_parts ; i++) {
		if (!strcmp(msm_nand_data.parts[i].name , "parameter")) {
			...
			param_end_block = msm_nand_data.parts[i].offset + 8;
		}
	  if (!strcmp(msm_nand_data.parts[i].name, "MIBIB")) {
	   cp_area_start_page = ((msm_nand_data.parts[i].offset) * 64 * 4096);
	  }
	  if (!strcmp(msm_nand_data.parts[i].name, "boot")) {
	   cp_area_end_page = ((msm_nand_data.parts[i].offset * 64 * 4096) - 1);
	  }
	}
#endif
			     ...
			     struct mtd_oob_ops *ops)
{
	...
	while (page_count-- > 0) {
		...
		for (n = start_sector; n < cwperpage; n++) {
			...
			if (ops->datbuf) {
				if (ops->mode != MTD_OOB_RAW)
					if(modem_partition) sectordatasize = 512;
					else sectordatasize = (n < (cwperpage - 1)) ? 516 : (512 - ((cwperpage - 1) << 2));
				else sectordatasize = 528;
			...
			{
				cmd->cmd = 0;
				if ((n == (cwperpage - 1)) && (!modem_partition))
				{
					cmd->src = MSM_NAND_FLASH_BUFFER + (512 - ((cwperpage - 1) << 2));
					sectoroobsize = (cwperpage << 2);
					
					if (ops->mode != MTD_OOB_AUTO) sectoroobsize += 10;
				} 
				else 
				{
	...
	while (page_count-- > 0) {
		...
		for (n = 0; n < cwperpage ; n++) {
			...
			if (n == 0) {
				...
				/* write data block */
			if (ops->mode != MTD_OOB_RAW)
				if(modem_partition) sectordatawritesize =  512;
				else sectordatawritesize = (n < (cwperpage - 1)) ? 516 : (512 - ((cwperpage - 1) << 2));
			else sectordatawritesize = 528;
			...
			if (ops->oobbuf) {
				if ((n == (cwperpage - 1)) && (!modem_partition))
				{
					...
					cmd->dst = MSM_NAND_FLASH_BUFFER + (512 - ((cwperpage - 1) << 2));
					
					if ((cwperpage << 2) < oob_len) cmd->len = (cwperpage << 2);
					else cmd->len = oob_len;
...
void msm_write_param(struct ariesve_parameter *param_data)
{
	...
	param_erase_info->addr = (param_start_block) * 64 * 4096;
	...
	memset(data_buf,0x0,4096);
	memset(spare_buf,0xFF,128);
	memcpy(data_buf,param_data,sizeof(struct ariesve_parameter));
	...
	ret =  msm_nand_write_oob(current_mtd, ((param_start_block) * 64)*4096, &ops);
...
void msm_read_param(struct ariesve_parameter *param_data)
{
	...
	if (msm_nand_block_isbad(current_mtd, ((param_start_block) * 64)*4096))
		param_start_block++;
	...
	ret =  msm_nand_read_oob(current_mtd, ((param_start_block) * 64)*4096, &ops);
	memcpy(param_data,data_buf,sizeof(struct ariesve_parameter));
...
int msm_nand_scan(struct mtd_info *mtd, int maxchips)
{
	...
	//CP partition--------------------------------------------------
	CFG0_M = (((mtd_writesize >> 9)-1) << 6) /* 4/8 cw/pg for 2/4k */
	...
	pr_info("nandcfg(Apps): %x %x %x(used)\n", CFG0_A, CFG1_A, ECC_CFG_A);	
	pr_info("nandcfg(Modem): %x %x %x(used)\n", CFG0_M, CFG1_M, ECC_CFG_M);
#else
...
static int __devinit msm_nand_probe(struct platform_device *pdev)
{
	...
	if (!interleave_enable) {
		if (!dual_nand_ctlr_present)
			pr_info("%s: Single Nand Ctrl in ping-pong mode\n", __func__);
		else
			pr_info("%s: Dual Nand Ctrl in ping-pong mode\n", __func__);
	}
	else {
		if (!dual_nand_ctlr_present)
			pr_info("%s: Single Nand Ctrl in interleave mode\n", __func__);
		else
