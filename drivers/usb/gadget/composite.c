...
{
	unsigned long			flags;
	printk("usb_composite_force_reset  ENTRY \n" );
	...
	if ( 1 ){
		...
		printk("disconnect usb cdev->gadget.name=%s \n" , cdev->gadget->name);		
		usb_gadget_disconnect(cdev->gadget);
		msleep(10);
		CSY_DBG_ESS("connect usb again\n");
		printk("connect usb again  cdev->gadget.name=%s \n" , cdev->gadget->name);		
